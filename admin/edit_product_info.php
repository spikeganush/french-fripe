...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(!empty($name_product)){
        echo $quantity_product;
         ...
         $sql = "UPDATE products SET name=?, type=?, categories_id=?, price=?, quantity=?, short_description=?, full_description=?, minimum_quantity=?, weight=?, pieces_by_box=?, online=? WHERE id=?";
        
         
         if($stmt = mysqli_prepare($connection, $sql)){
             // Bind variables to the prepared statement as parameters
             mysqli_stmt_bind_param($stmt, "siiiissidiii", $name_product, $type_product, $category_product, $price_product, $quantity_product, $short_description_product, $long_description_product, $minimum_quantity_product, $weight_product, $pieces_by_box_product, $online, $product_id);
                
             
             // Attempt to execute the prepared statement
             if(mysqli_stmt_execute($stmt)){
                 // Redirect to login page
                 header("location: upload_photo_edit.php?id=$product_id");
                ...
                <div class="col__right">
                    ...
                    <div class="row">
                        ...
                        <div class="product_title">
                            <a href="admin_products.php" class="add_product_select">
                        ...
                        <div class="App">
                            <div class="vertical-center">
                                <div class="inner-block">
                                    <form action="edit_product_info.php?id=<?php echo htmlspecialchars($product_id);?>"
                                        method="post">
                                        <h3>Details</h3>
                                        ...
                                        <?php
                                                ...
                                                while($row_showProducts = mysqli_fetch_array($query_products)) {
                                                    ?>
                                        <div class="form-group">
                                            <label>Name product</label>
                                            <input type="text" class="form-control" name="name_product"
                                                value="<?php echo htmlspecialchars($row_showProducts["name"]);?>" />
                                        </div>
                                        ...
                                        <select name="type_product">
                                            ...
                                            while($row_type = mysqli_fetch_array($query_type)) {                           
                                                ...
                                                if($row_type["id"] == $row_showProducts["type"]){

                                                                                                    ?>
                                            <option value="<?php echo htmlspecialchars($row_type["id"]);?>" selected>
                                                <?php echo htmlspecialchars($row_type["name"]);?></option>

                                            <?php                                                        
                                                                                            } else {
                                                                                                    ?>
                                            <option value="<?php echo htmlspecialchars($row_type["id"]);?>">
                                                <?php echo htmlspecialchars($row_type["name"]);?></option>
                                            <?php    
                                        ...
                                        <select name="category_product">
                                            <?php
                                                                    ...
                                                                    while($row_categories_form = mysqli_fetch_array($query_categories_form)) {                           
                                                                                        ...
                                                                                        if($row_categories_form["id"] == $row_showProducts["categories_id"]){
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" name="price_product"
                                                value="<?php echo htmlspecialchars($row_showProducts["price"]);?>" />
                                        </div>
                                        <div class="form-group">
                                            <label>Quantity in Stock</label>
                                            <input type="number" class="form-control" name="quantity_product"
                                                value="<?php echo htmlspecialchars($row_showProducts["quantity"]);?>" />
                                        </div>
                                        <div class="form-group">
                                            <label>Short description</label>
                                            <textarea rows="4" cols="50" class="form-control"
                                                name="short_description_product" /><?php echo htmlspecialchars($row_showProducts["short_description"]);?></textarea>
                                        </div>
                                        <div class="form-group">
                                            <label>Long description</label>
                                            <textarea rows="15" cols="50" class="form-control"
                                                name="long_description_product" /><?php echo htmlspecialchars($row_showProducts["full_description"]);?></textarea>
                                        </div>
                                        <div class="form-group">
                                            <label>Minimum quantity the client can order</label>
                                            <input type="number" class="form-control" name="minimum_quantity_product"
                                                value="<?php echo htmlspecialchars($row_showProducts["minimum_quantity"]);?>" />
                                        </div>
                                        <div class="form-group">
                                            <label>Average weight for one piece in Kg</label>
                                            <input type="number" step="0.001" class="form-control" name="weight_product"
                                                value="<?php echo htmlspecialchars($row_showProducts["weight"]);?>" />
                                        </div>
                                        <div class="form-group">
                                            <label>Average pieces in one box</label>
                                            <input type="number" class="form-control" name="pieces_by_box_product"
                                                value="<?php echo htmlspecialchars($row_showProducts["pieces_by_box"]);?>" />
                                        </div>

                                        <div class="form-group">
                                            <label>Online?</label>
                                            <label class="online"
                                                for="<?php echo htmlspecialchars($row_showProducts["id"]); ?>">
                                                <?php
                                    if($row_showProducts["online"] == 1){
                                        ?>
                                                <input class="online__input" name="online_status" type="checkbox"
                                                    id="<?php echo htmlspecialchars($row_showProducts["id"]); ?>"
                                                    checked>
                                                <?php
                                    ...
                                    ?>
                                        ...
                                        <button type="submit" name="save"
                                            class="btn-outline-primary btn-lg btn-block">Save and go to photo</button>
                                    </form>
